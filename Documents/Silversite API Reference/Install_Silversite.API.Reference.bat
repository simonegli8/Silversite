...
HelpLibraryManagerLauncher.exe /product "VS" /version "100" /locale en-us /uninstall /silent /vendor "silversite_org" /productName "Silversite API Reference" /mediaBookList "Silversite API Reference" > NUL
...
REM build time.
IF EXIST "Silversite.API.Reference.msha" COPY /Y "Silversite.API.Reference.msha" HelpContentSetup.msha
...
REM Uninstall first in case it is already there.  If not, it won't install below.  We'll ignore any error output
REM by redirecting it to NUL.
HelpLibraryManagerLauncher.exe /viewerVersion 2.0 /catalogName VisualStudio11 /locale en-us /wait 0 /operation uninstall /vendor "silversite_org" /productName "Silversite API Reference" /bookList "Silversite API Reference" > NUL

REM Install the new content.
HelpLibraryManagerLauncher.exe /viewerVersion 2.0 /catalogName VisualStudio11 /locale en-us /wait 0 /operation install /sourceUri "%CD%\Silversite.API.Reference.msha"
