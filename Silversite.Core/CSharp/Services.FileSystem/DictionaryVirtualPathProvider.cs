...
[assembly: Silversite.Services.DependsOn(typeof(Silversite.Services.DictionaryVirtualPathProvider))]
[assembly: Silversite.Services.DependsOn(typeof(Silversite.Services.VirtualPathRewriteDefault))]
...
namespace Silversite.Services {
	...
	/// </summary>
	public class DictionaryVirtualPathProvider: System.Web.Hosting.VirtualPathProvider, IAutostart {
...
#if DEBUG
		public string DisplayFiles { // for debugging, shows all entries
			get { return FilesAndDirectories.Select(f => f.VirtualPath).StringList("\r\n"); }
		}

		public string VirtualSize {
			get {
				var assemblies = FilesAndDirectories
					.OfType<ResourceVirtualFile>()
					.GroupBy(f => f.Assembly);
				return assemblies
					.Select(a => string.Format("{0}: {1}KB", a.Key.DisplayName(), a.Sum(f => f.Open().Seek(0, SeekOrigin.End))/1024))
		...
		public static string DirectoryName(string path) {
			path = Services.Paths.Absolute(path);
			int slash = path.LastIndexOf('/', path.Length > 2 ? path.Length-2 : 0);
			return path.Substring(0, slash > 0 ? slash : 0);
		}
		internal void CheckLazyPath(string path) {		
			// look for lazy path assemblies.
			Services.Lazy.Paths.Load(path);
			//foreach (var assembly in assemblies) AppVirtualFiles.Current.LoadAssembly(assembly); // load lazy path assembies.
		}
		/// <summary>
			...
			lock (FilesAndDirectories) {
				...
				if (!FilesAndDirectories.Contains(path)) {
					...
					try {
						...
						Services.Log.Error("Internernal Error: directory.VirtualPath:{0}", ex, path);
						Debugger.Break();
					}
				}
			}
			...
			lock (FilesAndDirectories) {
				...
				if (!FilesAndDirectories.Contains(path)) {
					...
					try {
						...
						Services.Log.Error("Internal Error: file.VirtualPath:{0}", ex, file.VirtualPath);
						Debugger.Break();
					}
				}
			}
		...
		public virtual void RegisterWithReflection() {
			...
			mi.Invoke(hostingEnvironmentInstance, this);
		}
		/// <summary>
		...
		public void Startup() {
			Current = this;
			Web.Modules.DependsOn<VirtualFiles>();
			Register();
		}
	}
	/// <summary>
	...
	/// </summary>
	public class VirtualPathRewriteDefault: Web.IHttpAutoModule {
		/// <summary>
		...
		/// <param name="sender">The sender.</param>
		/// <param name="e">The EventArgs.</param>
		public void MapStatic(object sender, EventArgs e) { // TODO: does not work yet.
			var context = ((HttpApplication)sender).Context;
			string path;
			if (HostingEnvironment.ApplicationVirtualPath != "/") {
...
//TODO				context.Handler = new System.Web.StaticFileHandler();
	...
	public class VirtualDirectory: System.Web.Hosting.VirtualDirectory, ICacheableObject {
		...
		string PathWithSlash(bool dir) {
			...
			if (!LoadedOnly) {
				foreach (var p in Lazy.Paths) {
					var lpath = Paths.Absolute(p.Path);
					var dpath = string.Empty;
					if (dir && lpath.EndsWith("/*")) dpath = lpath.Substring(0, lpath.Length - 2);
					if (IsChild(lpath, path) || (dir && IsChild(dpath, path))) {
						p.Assemblies.ForEach(a => a.Load());
					}
				}
			}
			return path;
...
#pragma warning restore
	...
	public class VirtualFiles: IAutoLoader {
...
#pragma warning restore
				Log.Error("Error adding virtual resource file", ex, Log.ErrorClass.Severe);
			}
		}
		/*
		...
		public void Startup() {
			...
			Modules.DependsOn<DictionaryVirtualPathProvider>();
			Modules.DependsOn<Services.Lazy>();
			//var provider = DictionaryVirtualPathProvider.Current;
