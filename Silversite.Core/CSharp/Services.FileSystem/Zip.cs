...
namespace Silversite.Services {
	...
	public class Zip: StaticService<Zip, ZipProvider> {
		public static void Create(string paths, string file, Func<string, bool> filter = null, ProgressEventHandler progressHandler = default(ProgressEventHandler)) {
			Provider.Create(paths, file, filter, progressHandler);
		}
		public static void Create(string paths, IEnumerable<string> files, string file, ProgressEventHandler progressHandler = default(ProgressEventHandler)) {
			Provider.Create(paths, files, file, progressHandler);
		}
		public static void Extract(string file, string to, Func<string, bool> filter = null, ProgressEventHandler progressHandler = default(ProgressEventHandler)) {
			Provider.Extract(file, to, filter, progressHandler);
		}
		public static IEnumerable<string> Content(string file) {
			return Provider.Content(file);
		}
	}

	public class ZipProvider: Provider<Zip> {

		public void Create(string paths, string file, Func<string, bool> filter = null, ProgressEventHandler progressHandler = default(ProgressEventHandler)) {
			using (var zip = ZipStorer.Create(Paths.Map(file), "Generated by Silversite")) {
				zip.EncodeUTF8 = true;
				if (progressHandler != default(ProgressEventHandler)) zip.Progress += progressHandler;
				zip.AddAll(paths, filter);
			}
		}

		public void Create(string paths, IEnumerable<string> files, string file, ProgressEventHandler progressHandler = default(ProgressEventHandler)) {
			using (var zip = ZipStorer.Create(Paths.Map(file), "Generated by Silversite")) {
				zip.EncodeUTF8 = true;
				if (progressHandler != default(ProgressEventHandler)) zip.Progress += progressHandler;
				zip.AddAll(paths, files);
			}
		}
		public void Extract(string file, string to, Func<string, bool> filter = null, ProgressEventHandler progressHandler = default(ProgressEventHandler)) {
			using (var zip = ZipStorer.Open(Paths.Map(file), System.IO.FileAccess.Read)) {
				if (progressHandler != default(ProgressEventHandler)) zip.Progress += progressHandler;
				zip.ExtractAll(to, filter);
			}
		}
		public IEnumerable<string> Content(string file) {
			using (var zip = ZipStorer.Open(Paths.Map(file), System.IO.FileAccess.Read)) {
				return zip.ReadCentralDir().Select(de => de.FilenameInZip);
			}
		}
	}
...
namespace System.IO.Compression {
	...
	using Silversite.Services;
	 ...
	 public class ZipStorer : IDisposable
	 {
		  ...
		  {
				Stream stream = new FileStream(Paths.Map(_filename), FileMode.Create, FileAccess.ReadWrite);
		  ...
		  {
				Stream stream = (Stream)new FileStream(Paths.Map(_filename), FileMode.Open, _access == FileAccess.Read ? FileAccess.Read : FileAccess.ReadWrite);
		  ...
		  public void AddFile(Compression _method, string _pathname, string _filenameInZip = null, string _comment = "")
		  {
				if (Access == FileAccess.Read)
					 throw new InvalidOperationException("Writing is not alowed");

				FileStream stream = new FileStream(Paths.Map(_pathname), FileMode.Open, FileAccess.Read);
			  if (_filenameInZip == null) _filenameInZip = _pathname;
		...
		public void AddAll(string paths, Func<string, bool> filter = null) {
			var files = new PipeQueue<string>();
			var t = Tasks.Do<int>(() => {
				int total = 0;
				using (files) Silversite.Services.Files.All(paths).Each(file => { total++; files.Enqueue(file); });
				return total;
			});
			AddPipe(paths, files, t);
		}

		public void AddAll(string paths, IEnumerable<string> files) {
			var queue = new PipeQueue<string>();
			var t = Tasks.Do<int>(() => {
				int total = 0;
				files.Each(file => { total++; queue.Enqueue(file); });
				return total;
			});
			AddPipe(paths, queue, t);
		}

		private void AddPipe(string paths, PipeQueue<string> files, System.Threading.Tasks.Task<int> t) {
			int total = 0, n = 0;
			var args = new ProgressEventArgs();
			var roots = paths.Tokens();
			foreach (var file in files) {
				var root = roots.Select(path => path.Contains('*') ? Paths.Directory(path) : path)
					.FirstOrDefault(path => file.StartsWith(path));
				if (root == null) root = "";
				AddFile(ZipStorer.Compression.Deflate, file, file.Substring(root.Length), "");
				n++;
				if (Progress != null && files.Closed) {
					total = t.Result;
					var p = n / total;
					args.ProcessedFiles = n;
					args.TotalFiles = total;
					args.Progress = p;
					Progress(this, args);
				}
			}
		...
		public void ExtractAll(string to, Func<string, bool> filter = null) {
			if (to.IsNullOrWhiteSpace()) throw new NotSupportedException("No destination specified for unzipping.");
			var files = ReadCentralDir();
			var args = new ProgressEventArgs();
			int total = files.Count, n = 0;
			foreach (var entry in ReadCentralDir()) {
				ExtractFile(entry, Paths.Combine(to, entry.FilenameInZip.Replace('\\', '/')));
				if (Progress != null) {
					var p = n / total;
					args.ProcessedFiles = n;
					args.TotalFiles = total;
		  ...
		  public bool ExtractFile(ZipFileEntry _zfe, string _filename)
		  {
				...
				string path = System.IO.Path.GetDirectoryName(Paths.Map(_filename));
				...
				if (Directory.Exists(_filename))
				 ...
				 Stream output = Silversite.Services.Files.Write(_filename);
