...
namespace Silversite.Services {
	...
	/// CopyTo will copy fewer than Count elements in this situation.
	/// </remarks>
	public sealed class WeakValueDictionary<TKey, TValue> : BaseDictionary<TKey, TValue>
		where TKey : class
		where TValue : class {
		...
		public WeakValueDictionary(int capacity, IEqualityComparer<TKey> comparer) {
			this.comparer = new WeakKeyComparer<TKey>(comparer);
			this.dictionary = new Dictionary<TKey, WeakReference<TValue>>(capacity, this.comparer);
		}
		...
		public override void Add(TKey key, TValue value) {
			if (key == null) throw new ArgumentNullException("key"); 
			WeakReference<TValue> weakValue = WeakReference<TValue>.Create(value);
			this.dictionary.Add(key, weakValue);
		}
		...
		protected override void SetValue(TKey key, TValue value) {
			this.dictionary[key] = WeakReference<TValue>.Create(value);
		}
		...
		public override IEnumerator<KeyValuePair<TKey, TValue>> GetEnumerator() {
			foreach (KeyValuePair<TKey, WeakReference<TValue>> kvp in this.dictionary) {
				WeakReference<TValue> weakValue = kvp.Value;
				TKey key = kvp.Key;
				TValue value = weakValue.Target;
		...
		public void RemoveCollectedEntries() {
			...
			foreach (KeyValuePair<TKey, WeakReference<TValue>> pair in this.dictionary) {
				WeakReference<TValue> weakValue = pair.Value;
