...
namespace Silversite.Web.HttpCompress
{
    /// <summary>
                ...
                foreach (Match match in Regex.Matches(html, _cssPattern, RegexOptions.IgnoreCase))
                {
                    ...
                    if (uri.Host == baseUri.Host)
                    {
                        ...
                        if (uri.AbsolutePath.ToLower().StartsWith(app.Context.Request.ApplicationPath.ToLower()))
                        {
                            ...
                            css[path].Add(file + (href.Contains("?") ? href.Substring(href.IndexOf("?")) : string.Empty));
                            // replace the origianl links with blanks
                            html = html.Replace(linkHtml, string.Empty);
                ...
                foreach (Match match in Regex.Matches(html, _jsPattern, RegexOptions.IgnoreCase))
                {
                    ...
                    if (uri.Host == baseUri.Host)
                    {
                        if (uri.AbsolutePath.ToLower().StartsWith(app.Context.Request.ApplicationPath.ToLower()))
                        {
                            ...
                            js[path].Add(file + (src.Contains("?") ? src.Substring(src.IndexOf("?")) : string.Empty));
                        }
                        else
                ...
                foreach (Match match in Regex.Matches(html, _jsPattern, RegexOptions.IgnoreCase))
                {
                    ...
                    if (uri.Host == baseUri.Host)
                    {
                        if (uri.AbsolutePath.ToLower().StartsWith(app.Context.Request.ApplicationPath.ToLower()))
                        {
                            ...
                            file = uri.AbsolutePath.Substring(index + 1) + (src.Contains("?") ? src.Substring(src.IndexOf("?")) : string.Empty);
                        }
                        else
                            file = uri.AbsolutePath + uri.Query;
                    }
