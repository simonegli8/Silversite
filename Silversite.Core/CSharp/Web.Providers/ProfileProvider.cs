...
namespace Silversite.Web.Providers {
	...
	//[Table("Silversite_Web_Providers_Profile")]
	...
	public class ProfileProvider: System.Web.Profile.ProfileProvider {
		...
		public override void Initialize(string name, NameValueCollection config) {
			...
			if (name == null || name.Length == 0)
				name = "SilversiteProfileProvider";

			if (string.IsNullOrEmpty(config["description"])) {
				config.Remove("description");
				config.Add("description", "Silversite Profile provider");
			}
			base.Initialize(name, config);

			try {
				string applicationName = GetConfigValue(config["applicationName"], HostingEnvironment.ApplicationVirtualPath);
				Application.Set<SilversiteProfileProvider>(applicationName);

				db = Data.Database.Custom(config["connectionStringName"]);
