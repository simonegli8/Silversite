 ...
 $.fn.splitter = function(args){
	...
	return this.each(function() {
		...
		function startSplitMouse(evt) {
			...
			if ( opts.outline ) {
				zombie = zombie || bar.clone(false).css("z-index", 99).insertAfter(A);
			...
			A._posSplit = A[0][opts.pxSplit] - evt[opts.eventPos] + dimSum(A, "margin-"+opts.side1.toLowerCase(), "margin-"+opts.side2.toLowerCase());
			$(document)
		...
		function resplit(pos) {
			...
			bar.css(opts.origin, pos).css(opts.sfixed, splitter._DF-bar._PMF);
			A.css(opts.origin, 0).css(opts.split, Math.max(pos-A._PMA, 0)).css(opts.sfixed,  Math.max(splitter._DF-A._PMF, 0));
			B.css(opts.origin, pos+bar._DA).css(opts.split, Math.max(splitter._DA-bar._DA-pos-B._PMA, 0)).css(opts.sfixed,  Math.max(splitter._DF-B._PMF, 0));
		...
		var opts = $.extend({
			...
			splitterClass:	"splitter-standard splitter ui-widget ui-widget-content",
			paneClass: "splitter-pane-standard splitter-pane",
			barClass: "splitter-bar-standard splitter-bar",
			barNormalClass: "ui-state-default-standard ui-state-default",			// splitbar normal
			barHoverClass: "ui-state-hover-standard ui-state-hover",			// splitbar mouse hover
			barActiveClass: "ui-state-highlight-standard ui-state-highlight",		// splitbar being moved
			barLimitClass: "ui-state-error-standard ui-state-error",			// splitbar at limit
			iframeClass: "splitter-iframe-hide-standard splitter-iframe-hide",		// hide iframes during split
			...
			v: {					// Vertical splitters:
				...
				barStateClass: "splitter-bar-vertical-standard splitter-bar-vertical",
				barDockedClass: "splitter-bar-vertical-docked-standard splitter-bar-vertical-docked"
			},
			h: {					// Horizontal splitters:
				keyTop: 40, keyBottom: 38,  cursor: "n-resize",
				barStateClass: "splitter-bar-horizontal-standard splitter-bar-horizontal",
				barDockedClass: "splitter-bar-horizontal-docked-standard splitter-bar-horizontal-docked"
			}
		}[vh], args, {
			...
			v: {					// Vertical splitters:
				...
				sfixed: "height", pxFixed: "offsetHeight", side3: "Top",  side4: "Bottom"
			},
			h: {					// Horizontal splitters:
				type: 'h', eventPos: "pageY", origin: "top",
				split: "height", pxSplit: "offsetHeight", side1: "Top",  side2: "Bottom",
				sfixed: "width",  pxFixed: "offsetWidth",  side3: "Left", side4: "Right"
			}
		}[vh]);
		...
		if ( /^(auto|default|)$/.test(bar.css("cursor")) && !opts.fixed )
			bar.css("cursor", opts.cursor);

		// Cache several dimensions for speed, rather than re-querying constantly
		// These are saved on the A/B/bar/splitter jQuery vars, which are themselves cached
		// DA=dimension adjustable direction, PBF=padding/border sfixed, PBA=padding/border adjustable
		bar._DA = bar[0][opts.pxSplit];
		bar.css("margin-top", dimSum(bar, "margin-top", "padding-top")).css("padding-top", 0)
			.css("margin-left", dimSum(bar, "margin-left", "padding-left")).css("padding-left", 0)
			.css("margin-right", dimSum(bar, "margin-right", "padding-right")).css("padding-right", 0)
			.css("margin-bottom", dimSum(bar, "margin-bottom", "padding-bottom")).css("padding-bottom", 0);
		bar._PMF = Math.max(dimSum(bar, "margin-" + opts.side3.toLowerCase(), "margin-" + opts.side4.toLowerCase()), dimSum(bar, "border" + opts.side3 + "Width", "border" + opts.side4 + "Width"));
		bar._PMA = Math.max(dimSum(bar, "margin-" + opts.side1.toLowerCase(), "margin-" + opts.side2.toLowerCase()), dimSum(bar, "border" + opts.side1 + "Width", "border" + opts.side2 + "Width"));
		...
		$.each([A,B], function(){
			...
			this.css("margin-top", dimSum(this, "margin-top", "padding-top")).css("padding-top", 0)
				.css("margin-left", dimSum(this, "margin-left", "padding-left")).css("padding-left", 0)
				.css("margin-right", dimSum(this, "margin-right", "padding-right")).css("padding-right", 0)
				.css("margin-bottom", dimSum(this, "margin-bottom", "padding-bottom")).css("padding-bottom", 0);
			this._PMF = dimSum(this, "margin-" + opts.side3.toLowerCase(), "margin-" + opts.side4.toLowerCase());
			this._PMA = dimSum(this, "margin-" + opts.side1.toLowerCase(), "margin-" + opts.side2.toLowerCase());
		...
		if ( opts.dock ) {
			splitter
				.bind("toggleDock"+opts.eventNamespace, function() {
					...
					splitter.trigger((pw?"dock":"undock")+opts.eventNamespace);
				})
